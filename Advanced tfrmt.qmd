...
This document summarizes and reproduces the R code that builds a formatted primary results table using the `tfrmt` package. The original script:

- Reads an Analysis Results Data (ARD) dataset from `datasets/ARD/model.xpt`.
- Defines a `tfrmt` object to structure the table by groups, row labels, and columns.
- Applies formatting (rounding, combining confidence intervals, conditional p‑value display).
- Adds Big N information, column ordering, row grouping, and footnotes.
- Generates a `gt` table, styles it, and saves it as HTML/DOCX.
- Also shows how to create mock tables for previewing the layout.
